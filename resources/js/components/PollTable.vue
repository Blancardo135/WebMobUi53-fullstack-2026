...
<template>
  ...
  <div v-else>
    <div v-if="polls.length === 0" class="flex flex-col items-center justify-center py-16 text-gray-400">
    ...
    <div v-else>
      ...
      <!-- j'aurais pu faire un composant PollCard.vue. Mais vu que je l'utilise
       qu'une seule fois, je n'ai pas jugé pertinent de l'implémenter. -->
      ...
      <!-- version desktop, responsivité -->
      <div class="hidden lg:block overflow-hidden rounded-xl border border-gray-200 shadow-sm">
        <table class="w-full text-sm text-left">
          ...
          <tbody class="divide-y divide-gray-100 bg-white">
            <tr v-for="poll in polls" :key="poll.id" class="transition-colors hover:bg-gray-50">
  ...
  <BaseModal v-if="pollToDelete" title="Confirmer la suppression"
    message="Cette action est irréversible. Le sondage sera définitivement supprimé." confirmLabel="Supprimer"
    confirm-variant="red" @confirm="deletePoll" @cancel="pollToDelete = null" />
</template>