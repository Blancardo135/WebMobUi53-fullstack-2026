    ...
    <div v-else>
      ...
      <ul class="space-y-2 mt-3">
        <li v-for="option in poll.options" :key="option.id">
          <button @click="changeOption(option.id)" :class="[
      ...
      <BaseButton variant="blue" @click="submitVote(() => emit('voted'))"
        :disabled="loading || selectedOptionIds.length === 0" class="mt-4 w-full">
